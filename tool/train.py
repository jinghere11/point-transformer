...
def main():
    ...
    if args.data_name == 's3dis':
        ...
        swDataset(split='train', data_root=args.data_root, test_area=args.test_area)
...
def train(train_loader, model, criterion, criterion_ntx, optimizer, epoch, T_k, swDataset):
    ...
    for i, (coord, feat, target, mask, offset) in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        mask_order_narrow = mask_order[:int(len(mask_order)*0.8)]
        ...
        # if epoch < 1:
        #     loss = criterion(output, target) + criterion(output_gcn[mask], target[mask]) + criterion(output_pt, target)
        # else:
        #     loss = criterion(output, target) + criterion(output_pt, target)  # + criterion(logits, labels)



        sorted, indices = torch.sort(output_gcn, descending=True)
 
        output_gcn_label = indices[:, 0]
        mask_gcn = torch.nonzero((output_gcn_label == target)).squeeze()
        # if len(mask_gcn.shape):
        #     mask_gcn = mask_gcn.tolist() + mask.tolist()
        # else:
        #     mask_gcn = [mask_gcn] + mask.tolist()



        # if epoch < 1:
        #     loss = criterion(output_gcn[mask], target[mask])*len(target)/len(mask)*0.5 + criterion(output, target)*0.5 + criterion(output_pt, target)
        # # elif 3 <= epoch < 12:
        ...
        #     loss = criterion(output_pt, target) + criterion(output, target)*0.5

        loss = criterion(output_pt[mask_order_narrow], target[mask_order_narrow])
